...
SwitchControl::SwitchControl(QWidget *parent)
    ...
    m_thumbColor(Qt::darkGray),
    m_disabledColor(190, 190, 190),
    m_background(Qt::color1)
{
    m_nX = 500;
    m_nY = 200;
    // 鼠标滑过光标形状 - 手型
...
//    // 创建左侧和右侧的标签
//    QLabel* leftLabel = new QLabel("关", this);
//    QLabel* rightLabel = new QLabel("开", this);

//    // 设置标签的位置
//    int labelWidth = 20; // 调整标签的宽度
//    leftLabel->setGeometry(m_nMargin, height() / 2 - m_nHeight / 2, labelWidth, m_nHeight);
//    rightLabel->setGeometry(width() - m_nMargin - labelWidth, height() / 2 - m_nHeight / 2, labelWidth, m_nHeight);
...
void SwitchControl::onTimeout()
{
    ...
    {
        m_nX += 2;
        if (m_nX >= width() - (m_nHeight + m_radius * 1.5))
        {
            m_timer.stop();
        }
    } else
